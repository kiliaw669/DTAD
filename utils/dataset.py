...
class Dataset_maker(torch.utils.data.Dataset):
    def __init__(self, root, category, config, is_train=True):
        self.image_transform = transforms.Compose(
            [
                transforms.Resize((config.data.image_size, config.data.image_size)),  # Resize data to rectangle.
        ...
        self.mask_transform = transforms.Compose(
            [
                ...
                transforms.ToTensor(), # Scales mask data into [0,1] 
            ]
        )
        if is_train:
            if category:
            ...
            if category:
                ...
                for c in categories:
                    self.image_files.extend(
                        glob(os.path.join(root, c, "train", "good", "*.png"))
                    )
        else:
            if category:
                self.image_files = glob(os.path.join(root, category, "test", "*", "*.png"))
                ...
                for c in categories:
                    self.image_files.extend(
                        glob(os.path.join(root, c, "test", "*", "*.png"))
                    )
        self.is_train = is_train
